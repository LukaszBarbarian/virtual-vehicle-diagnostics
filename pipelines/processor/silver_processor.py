...
class SilverProcessor(BaseProcessor):
    """
    SilverProcessor: Transforms raw JSON events from Bronze into a validated, 
    structured, and flattened Delta table. Implements Data Quality checks and 
    quarantine for invalid records.
    """

    def read(self) -> DataFrame:
        """
        Reads raw JSON telemetry events from the Bronze storage layer.
        """
        return (
            self.spark.read
            .format("json")
    ...
    def process(self, bronze_df: DataFrame) -> DataFrame:
        """
        Executes the main transformation pipeline: schema parsing, projection, 
        quality gate application, and dataset splitting.
        """
        ...
        # Side effects: record issues and log metrics
        self._write_quarantine(quarantine_df)
        self._write_metrics(checked)

        return valid_df

    def write(self, silver_df: DataFrame):
        """
        Persists validated telemetry data to the Silver Delta table, 
        partitioned by simulation_id for optimized query performance.
        """
        (
            silver_df
            .write
    ...
    def _parse_raw_event(self, bronze_df: DataFrame) -> DataFrame:
        """
        Maps the raw JSON string into a strictly defined Spark StructType schema.
        """
            ...
            StructField("payload", StructType([
                ...
                StructField("modules", StructType([
                    ...
                    StructField("driver", StructType([
                        ...
                        StructField("throttle_rate", DoubleType())
                    ])),
    ...
    def _project_silver(self, parsed_df: DataFrame) -> DataFrame:
        """
        Flattens the nested JSON structure into a tabular format, 
        normalizing column names for downstream ML features.
        """
        return (
            parsed_df
            .select(
    ...
    def _apply_quality_checks(self, df: DataFrame) -> DataFrame:
        """
        Evaluates data quality rules including null checks and physical range constraints.
        """
        return (
            df
            .withColumn(
                "is_null_violation",
                F.expr("simulation_id IS NULL OR step IS NULL OR engine_rpm IS NULL OR speed_kmh IS NULL")
            )
            .withColumn(
                "is_range_violation",
                F.expr("""
            ...
            .withColumn("is_logical_violation", F.lit(False))
            .withColumn(
                "is_valid",
    ...
    def _split_valid_quarantine(self, df: DataFrame) -> (DataFrame, DataFrame):
        """
        Splits the dataset into valid records for the Silver layer and failed records for quarantine.
        """
        valid_df = df.filter("is_valid = true").drop("is_null_violation", "is_range_violation", "is_logical_violation", "is_valid")
        quarantine_df = df.filter("is_valid = false")
        return valid_df, quarantine_df

    def _write_quarantine(self, quarantine_df: DataFrame):
        """
        Archives corrupted or out-of-bounds telemetry records for offline investigation.
        """
        (
            quarantine_df
            .write
    ...
    def _write_metrics(self, df: DataFrame):
        """
        Exports data quality metrics to a central monitoring table.
        """
        metrics_df = (
            df.groupBy()
            .agg(
